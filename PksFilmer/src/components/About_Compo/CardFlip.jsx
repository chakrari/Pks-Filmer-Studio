...
const CardFlip = () => {
  ...
  return (
    <div className="text-center grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-16 ml-12 gap-y-12 p-4">
      {aboutCard.map((card) => (
        <div
          key={card.id}
          onMouseEnter={() => setHoveredCard(card.id)}
          onMouseLeave={() => setHoveredCard(null)}
          className="relative perspective w-[50vh] h-[50vh]"
        >
          <div
          ...
          >
            ...
            <div className="absolute backface-hidden w-full h-full flex flex-col items-center justify-center border-8 border-white">
              <h2 className="font-abril font-bold text-2xl">
                {card.frontHeading}
              </h2>
              <p className="text-xl">{card.frontContent}</p>
            </div>
            {/* Back Side */}
